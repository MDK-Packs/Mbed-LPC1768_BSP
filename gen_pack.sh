...
"${PACKCHK}" "${PACK_BUILD}/${PACK_VENDOR}.${PACK_NAME}.pdsc" -i "${CMSIS_PACK_ROOT}/.Web/Keil.LPC1700_DFP.pdsc" -i "${CMSIS_PACK_ROOT}/.Web/ARM.CMSIS.pdsc" -i "${CMSIS_PACK_ROOT}/.Web/Keil.ARM_Compiler.pdsc" -n PackName.txt
errorlevel=$?
if [ $errorlevel -ne 0 ]; then
